...
import com.gigacloud.starcloud.common.constant.Constants;
import com.gigacloud.starcloud.common.core.text.StrFormatter;
...
public class StringUtils extends org.apache.commons.lang3.StringUtils
{
    ...
    public static String formatNumber (BigDecimal number) {
        ...
        if (number.compareTo(BigDecimal.ZERO) >= 0) {
            return "+" + NumberUtil.decimalFormatMoney(number.doubleValue());
        } else {
            return NumberUtil.decimalFormatMoney(number.doubleValue());
        }
    }
    ...
    public static boolean equalsIgnoreEmpty(String str1, String str2) {
        ...
        str1= null == str1 ? "": str1.trim();
        str2= null == str2 ? "" : str2.trim();
        return str1.equals(str2);
    }

    /**
     * 截取左侧指定长度字符串，不足长度的返回原串
     *
     * @param originStr 原始字符串
     * @param len       左侧截取长度，负数则从右侧开始截取
     * @return 左侧截取字符串
     */
    public static String subStrLeftByLen(String originStr, int len) {
        if (null == originStr) {
            return "";
        }
        ...
        int actualLen = Math.abs(len);
        if (strLen < actualLen) {
            return originStr;
        }

        if (0 > len) {
            return originStr.substring(strLen - actualLen);
        } else {
            return originStr.substring(0, actualLen);
        }
    }
}
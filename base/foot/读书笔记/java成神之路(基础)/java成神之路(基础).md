...
> 类变量被`static`修饰，属于类，生命周期等同于类的生命周期，当一个类被类加载器成功加载到方法区，其就已经存在于方法区。当类被卸载的时候，它也跟着消失。
...
- 第一种方式通过"字面量"的形式赋值，字符串如果在缓存池中不存在，则会创建并放入缓存池，并将引用返回
...
![image-20221102103801978](java成神之路(基础).assets/image-20221102103801978.png)
...
>
>  如果使用try catch进行捕获，在cache 代码块中没有对受检异常进行转化而直接向上抛出则必须在方法前使用throws声明受检异常。如下例子：
...
- throws     用于方法签名后，申明方法可能抛出的异常
- throw       后跟异常实例显示抛出异常
- try            用来包裹一块可能出现异常的代码块
- catch        跟在try代码后，捕获try中出现的异常
- finally        一些代码无论是否出现异常都会执行，可以定义在fianlly代码块中，一般用于资源关闭。finally代码块执行在整个方法完全返回之前(try 中的return    catch中的throw)，如果finally代码块中执行return或throw来终止方法，则会忽略之前的return 和 throw。

##### 异常申明

> 通过(throws)关键字进行异常声明，在方法签名后进行申明，如果存在多个异常则通过逗号隔开。
>
> 什么方法，何时需要声明？①当该方法调用了一个已经显示的申明了受检异常的方法，则必须申明。②希望调用该方法的方法必须处理对应异常，可显示申明受检异常。也可以申明非受检异常，但调用方法可不做处理
...
class Son extends Person {
    /**
     * 抛出比 FileNotFoundException 更广的异常则会报错 比如IOException
     *
     * @throws IOException
     */
    @Override
...
- 如果所调用的方法申明的是非受检异常，则可通过编译，但是运行期间程序出错则会终止程序运行

- 如果所调用的方法是受检异常，则调用方必须显示的try  catch 处理或throws 向上继续抛出。否则不可通过编译

- 异常申明尽量抛出任何可能出现的异常。 异常处理：如果不能准确处理，则必须继续向上抛出

  

##### 异常处理

> 当代码可能出现异常，则使用try语句包裹，使用catch进行捕获，可以继续向上抛出异常，也可以抛出自定义异常。
...
![image-20221102132446881](java成神之路(基础).assets/image-20221102132446881.png)

> 大部分情况下不需要手动抛出异常，Java大部分情况下都已经处理或申明了异常，所以一般都是向上抛出异常。
>
> 我们通过catch捕获异常，往往是将异常转化为系统统一异常，以此方便处理，并对外部系统隐藏实现细节，同时抛出异常时必须通过initCase()方法将异常原因保留，这样方便错误排查。
...
- try  catch
  ...
  > 可以捕获·多个不同类型的异常，并对不同类型的异常做不同的处理。
  >
  > 需要注意：①不可多次捕获同类型异常(这也是范型不可在异常中应用的原因) ②捕获异常范围不可由小到大(即不可先捕获子类，再捕获父类)

  ![image-20221102133904703](java成神之路(基础).assets/image-20221102133904703.png)

- try catch finally 

- try finally

  > 可以定义try  finally 。用于不用处理异常但是需要关闭资源的情况
...
> java7 开始提供了一个跟好的处理资源的方式：try-with-resources 语句。这是Java提供的语法糖，方便程序员编码，但是经过编译器编译后，都会转化成jvm认识的。
>
> 可以使用try  with  resource  关闭实现Closeable接口的资源
...
- 针对不正常情况
  ...
  >
  > 比如NPE(空指针异常)、(IndexOutOfBoundException)数组下标越界异常可通过代码解决，避免使用异常捕获。
...
> 反射是Java为程序员提供的可以在运行期间访问类信息的能力，反射很强大可以访问到一些不可见的类信息。通过反射可以获取字节码文件对应的Class对象。
...
- Class类(java.lang.Class)没有可供访问的构造器，它由类加载器加载，由虚拟机解析并创建与字节码文件唯一对应的Class对象
...
> Spring框架的演进，由XML配置到，JavaConfig配置再到注解自动配置。

##### Junit 3 -> 4

> Junit3 到Junit4的演化。
>
> Junit3继承，Junit4自定义@Test注解

##### AOP

> 使用注解配置连接点，通过切面实现代码解耦。
...
> 普通内部类和静态内部类，都属于语法糖。允许在一个类内部定义其他类，一但编译成功就会生成多份字节码。以下例子就会生成
>
...
可以发现会调用`LambdaMetafactory.metafactory`此方法，并且在第4、5个参数声明入参类型和出参数类型，在第四个参数指定方法实现。
